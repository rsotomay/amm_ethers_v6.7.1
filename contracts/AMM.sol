...
contract AMM {
    ...
    function addLiquidity(uint256 _token1Amount, uint256 _token2Amount) external {
        ...
        if(totalShares == 0) {
            ...
            require((share1 / 10**3) == (share2 / 10**3), "must provide equal token amount");
            share = share1;
        }
    ...
    // Returns amount of token1 recieved when swapping token2
    ...
    returns(uint256 token1Amount, uint256 token2Amount)
    {
        require(_share <= shares[msg.sender],'You dont hold that many shares');
